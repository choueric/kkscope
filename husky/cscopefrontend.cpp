	...
	ParserDelim /* ignored */)
{
	...
	case Searching:
		...
		if ((sscanf(sToken.toLatin1(), SEARCH_STR, &nFiles, &nTotal) == 2) ||
			(sscanf(sToken.toLatin1(), INV_STR, &nFiles, &nTotal) == 2) ||
			(sscanf(sToken.toLatin1(), REGEXP_STR, &nFiles, &nTotal) == 2)) {
			emit progress(nFiles, nTotal);
		}
	...
	case SearchEnd:
		...
		if ((sscanf(sToken.toLatin1(), SEARCHEND_STR, &nRecords) == 1) &&
			(m_nMaxRecords > 0) &&
...
void CscopeProgress::setProgress(int nProgress, int nTotal)
{
	...
	if (nProgress == nTotal) {
		...
		if (m_pLabel == NULL) {
            ...
            QPalette pe = m_pLabel->palette();
            pe.setColor(QPalette::Background, Plasma::Theme::HighlightColor);
            pe.setColor(QPalette::Foreground, Plasma::Theme::HighlightColor);
            m_pLabel->setPalette(pe);
	...
	if (m_pProgressBar == NULL) {
        ...
        m_pProgressBar->setMaximum(nTotal);
