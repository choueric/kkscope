	...
	m_pLastItem(NULL)
{
    ...
    setColumnCount(4);
    QStringList strList;
    strList << i18n("Function") << i18n("File") << i18n("Line") << i18n("Text");
 ...
 */
void QueryView::queryFinished(uint nRecords, QTreeWidgetItem*)
{
	// Auto-select a single record (no need to emit the show() signal in
	// that case)
	if (nRecords == 1) {
		emit select(topLevelItem(0));
		return;
	}
...
#if 0 // example for iterator
static void foreach_treewidget(QTreeWidget *p)
{
    QTreeWidgetItem *pItem;
    ...
    while (*it) {
        pItem = *it;
        qDebug() << pItem->text(0) << pItem->text(1);
        ++it;
    }
}
#endif
QueryView::Iterator QueryView::getIterator()
{
	...
	for (pItem = topLevelItem(0); pItem != NULL && !pItem->isVisible(); 
...
void QueryView::slotFilter(int nCol)
{
    ...
    while (*it) {
        pItem = *it;
		if (!pItem->isHidden() &&
			(re.indexIn(pItem->text(nCol)) == -1) != bNegate) {
