...
void CallTreeDlg::show()
{
    ...
    if (m_nDefView == 0) {
        m_pGraphButton->setChecked(true);
        m_pCalledButton->setChecked(false);;
        m_pCallingButton->setChecked(false);
    } else if (m_nDefView == 1) {
        m_pGraphButton->setChecked(false);
        m_pCalledButton->setChecked(true);;
        m_pCallingButton->setChecked(false);
    } else if (m_nDefView == 2) {
        m_pGraphButton->setChecked(false);
        m_pCalledButton->setChecked(false);;
        m_pCallingButton->setChecked(true);
    }
...
void CallTreeDlg::slotSaveClicked()
{
	...
	sFile = KFileDialog::getSaveFileName(); // TODO add filter or prefix path
